/**
 * P2P Service V2 for Electron
 * Implements seamless one-click model hosting using Socket.io + Simple-peer
 */
...
try {
  ...
  console.log('✅ Native WebRTC module loaded successfully');
} catch (error) {
  console.warn('⚠️ Native WebRTC module not available:', error.message);
  console.warn('This is required for P2P connections in Electron');
  wrtc = undefined;
}
...
class P2PServiceV2 {
  ...
  async initialize(userId, authToken) {
    console.log('🔧 P2P Service: initialize() called');
   ...
   */
  async enableHosting() {
    console.log('🟢 P2P Service: enableHosting() called');
   ...
   */
  async startHosting() {
    console.log('🚀 P2P Service: startHosting() called');
  ...
  async handleWebRTCSignal(data) {
    ...
    if (!peer || peer.destroyed) {
      ...
      if (this.connectingPeers.has(fromUserId)) {
        console.log(`⏳ Already connecting to ${fromUserId}, waiting...`);
  ...
  async createPeerConnection(userId, initiator) {
    ...
    if (pc) {
      ...
      pc.addEventListener('icegatheringstatechange', () => {
        ...
        // When gathering is complete, check what candidates we got
        if (pc.iceGatheringState === 'complete') {
          pc.getStats().then(stats => {
            let localCandidates = { host: 0, srflx: 0, relay: 0, prflx: 0 };
            let remoteCandidates = { host: 0, srflx: 0, relay: 0, prflx: 0 };
            
            stats.forEach(stat => {
              if (stat.type === 'local-candidate') {
                localCandidates[stat.candidateType] = (localCandidates[stat.candidateType] || 0) + 1;
              } else if (stat.type === 'remote-candidate') {
                remoteCandidates[stat.candidateType] = (remoteCandidates[stat.candidateType] || 0) + 1;
              }
            });
            
            console.log(`📊 Local candidates for ${userId}:`, localCandidates);
            console.log(`📊 Remote candidates for ${userId}:`, remoteCandidates);
            
            if (localCandidates.relay === 0) {
              console.error(`❌ No TURN relay candidates generated for ${userId} - mobile connection will likely fail!`);
            }
          });
        }
      });
      ...
      pc.addEventListener('iceconnectionstatechange', () => {
        ...
        if (pc.iceConnectionState === 'failed' || pc.iceConnectionState === 'disconnected') {
          pc.getStats().then(stats => {
            console.log(`📊 Connection stats for failed connection with ${userId}:`);
            let hasSuccessfulPair = false;
            ...
            if (!hasSuccessfulPair) {
              console.error('❌ No successful candidate pairs found - NAT traversal failed');
            }
          });
        }
      });
    ...
    const connectionTimeout = setTimeout(() => {
      if (!peer.connected && !peer.destroyed) {
        ...
        console.log(`Connection state: connected=${peer.connected}, destroyed=${peer.destroyed}`);
        ...
        if (pc) {
          pc.getStats().then(stats => {
            ...
            console.log(`📊 Had relay candidates: ${hasRelay}`);
          });
        }
          ...
          pc.getStats().then(stats => {
            stats.forEach(stat => {
              if (stat.type === 'candidate-pair' && stat.state === 'failed') {
                console.error('Failed candidate pair:', stat);
              }
            });
          });
        }
    ...
    peer.on('error', (error) => {
      ...
      // For ICE connection failures, attempt retry with mobile networks
      if (error.code === 'ERR_ICE_CONNECTION_FAILURE') {
        console.log(`🔄 ICE connection failed with ${userId}, will retry if peer reconnects`);
        // Don't immediately close - let timeout handle it
      } else {
        this.closePeerConnection(userId);
      }
    });
   ...
   */
  async sendRequestToPeer(userId, messageType, data = {}, timeout = 10000) {
    const peer = this.peers.get(userId);
    if (!peer || !peer.connected) {
      throw new Error(`Not connected to peer ${userId}`);
    }
...
const p2pServiceV2 = new P2PServiceV2();

module.exports = p2pServiceV2;